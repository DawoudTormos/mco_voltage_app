...
unsigned int timeToBeOFF = 0;
 bool timerToBeOFF = 0;


unsigned int disconnected = 3;
float low = 472.30003;//470
 float high =539.85;//
...
unsigned long int last_server_req=0;//time of last voltage update 
unsigned long int next_server_req_state=60000;
bool gen = 0;
...
if(gen==1 && timerToBeOFF && timeToBeOFF != 0 && millis() > timeToBeOFF ){
gen = 0;
timerToBeOFF = 0;
timeToBeOFF = 0;
}
...
if( millis() > next_server_req_state  ){
next_server_req_state = millis() + 60000;
...
if(voltage>low && voltage<high && gen==0 && timerToBeOFF == 0 && time_since_high==0 && time_since_low==0 && millis() > /*10000*/1000483648){
noInterrupts ();
timer0_millis = 0;
interrupts ();
delay(5);
...
void timerToGenOFF(unsigned int xx ){

timeToBeOFF = millis() + xx;
timerToBeOFF = 1 ;
}
